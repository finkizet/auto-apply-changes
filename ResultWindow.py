...
def apply_otd_adjustments(self):
    ...
    delta_x_tablet = dx_ratio * w
    delta_y_tablet = dy_ratio * h
    ...
    def _round_three(value):
        try:
            return round(float(value), 3)
        except (TypeError, ValueError):
            return value

    rounded_tablet = {key: _round_three(val) for key, val in updated_tablet.items()}

    output_config = copy.deepcopy(self.otd_config) if isinstance(self.otd_config, dict) else {}

    def _find_tablet_section(config):
        if not isinstance(config, dict):
            return None

        potential_tablets = []
        if "Tablet" in config:
            potential_tablets.append(config.get("Tablet"))
    ...
    output_tablet = _find_tablet_section(output_config)

    if output_tablet is None:
        QtWidgets.QMessageBox.critical(
            self,
            "Save failed",
            "Could not locate a Tablet section to update in the configuration copy.",
        )
        return

    output_tablet.update(rounded_tablet)
